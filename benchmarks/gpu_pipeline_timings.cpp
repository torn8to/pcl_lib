...
int main() {
  ...
  for (unsigned int i = start; i < iterations; ++i) {
    ...
    auto duration_longlong = iteration_timer.stop();
    double time_double = static_cast<double>(duration_longlong) / 1000000;
    timings.push_back(time_double);
    swa.update(time_double);
