...
namespace {                                                                        
  ...
  void transform_inplace(thrust::device_vector<Eigen::Vector3d> data, const Sophus::SE3d transform){
      thrust::transform(thrust::device,
                        ...
                        data.end(),
                        data.begin(),
                        [transform] __device__ (Eigen::Vector3d const& point){
...
Correspondences DataAssociation(thrust::device_vector<Eigen::Vector3d> &points, 
  ...
  thrust::copy_if(
    thrust::device,
    thrust::make_zip_iterator(thrust::make_tuple(points.begin(), closest_neighbor.begin(), distances.begin())),
    thrust::make_zip_iterator(thrust::make_tuple(points.end(), closest_neighbor.end(), distances.end())),
...
LinearSystem BuildLinearSystem(Correspondences &correspondences, const double kernel_scale) {
  auto compute_jacobian_and_residual = [] __device__ (const auto &correspondence) {
    ...
    LinearSystem result = thrust::reduce(
                  linear_systems.begin(),
                  linear_systems.end(),
                  LinearSystem(),
...
Sophus::SE3d GPURegistration<PointsPerVoxel,InitialCapacity>::alignPointsToMap(
    ...
    for(int i = 0; i < max_num_iterations_; i++){
      Correspondences correspondences = DataAssociation<PointsPerVoxel,InitialCapacity>(device_points,
                                                                                        voxel_map,
                                                                                        this,
