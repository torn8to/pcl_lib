...
struct VoxelKey_hash{
  __host__ __device__ std::size_t operator()(Eigen::Vector3i const& point){
    return static_cast<std::size_t>(point.x() * 83492791 ^ point.y() 
                                    * 6291469 ^ point.z() * 12582917);
  }
};
...
class GPUVoxelHashMap{
  public:
    GPUVoxelHashMap(double voxel_resolution = 1.0, int initial_capacity = 100001, double max_range = 100):
                resolution_(voxel_resolution),
                capacity_(initial_capacity),
                max_range_(max_range),
                map_(initial_capacity,
                     cuco::empty_key<Key>{Eigen::Vector3d{0, 0, 0}},
    ...
    //std::pair<Eigen::Vector3d> firstNearestNeighborQuery(Eigen::Vector3d const& point); moved this method to be A Kernel In GPURegistration
